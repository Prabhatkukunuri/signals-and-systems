...
\author{Prabhat kukunuri\\EE23BTECH11034}
\date{}

\maketitle
%-------------------------------------------------------------------------------
\section{Introduction}
We're assigned the creation of FIR and IIR filter designs confined to a particular filter type. Our focus is on creating a bandpass filter, and we've been provided with the specifications for this filter.
...
\begin{align}
    ...
    j&=7
\end{align}
By substituting $j = 7$, we determine the passband range for our bandpass filter as $8.2$ kHz to $9.4$ kHz.\\
...
In the bilinear transform, the analog filter frequency ($\Omega$) is related to the corresponding digital filter frequency ($\omega$) by the equation:
\begin{align}
...
To design filters with a monotonic stopband and equiripple passband, we utilize the Chebyshev approximation method to design our bandpass IIR filter.
\subsection{The Analog Filter}
\begin{enumerate}

\item {Low Pass Filter Specifications:}  Let $H_{a, BP}(j\Omega)$ represent the desired analog bandpass filter, incorporating the specifications outlined in Section 2.2. Additionally, let $H_{a,LP}(j\Omega_L)$ denote the equivalent low pass filter.
\begin{equation}
...
\begin{lstlisting}
https://github.com/Prabhatkukunuri/signals-and-systems/blob/main/filter%20design/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{figure}
In Figure \ref{fig:H_for_diff_eb}, we can observe equiripple behavior in the passband and monotonic behavior in the stopband.\\
As the value of $\epsilon$ increases, the magnitude of $\vert H_{a,LP}(j\Omega_L)\vert$ decreases.

\item {The Low Pass Chebyschev Filter:} The next step in the design process is to derive an expression for the magnitude response in the $s$-domain for the low pass Chebyshev filter
...
\begin{lstlisting}
https://github.com/Prabhatkukunuri/signals-and-systems/blob/main/filter%20design/codes/sk_gen.c
\end{lstlisting}
...
Therefore, the magnitude response is expressed as:
\begin{align}
...
\item {The Band Pass Chebyschev Filter:}
After verifying the design with the required specifications, the next step is to transition to the desired type of filter using frequency transformation.
\begin{align}
...
\end{align}
The expressions in the s-domain and gain factors are computed using a Python code.
\\
In Figure 3, we plot $\vert H_{a,BP}(j\Omega)\vert$ as a function of $\Omega$ for both positive and negative frequencies.\\
...
\end{align}
From equation \eqref{eq:h(n)_for_LPF}, we can infer that $h\brak{n}$ for an ideal Low Pass Filter is not causal, and it cannot be made causal by introducing a finite delay. \\
Additionally, $h\brak{n}$ does not converge, resulting in an unstable system.
\\
To address this issue, we employ windowing on the impulse response. A window function is selected and multiplied. The Kaiser window is chosen for this purpose.
\begin{align}
    w(n) =
    \begin{cases}
...
A Band-Pass Filter (BPF) is derived by subtracting the magnitude response of a Low-Pass Filter (LPF) with a cutoff frequency of $\omega_{p1}$ from another LPF magnitude response with a cutoff frequency of $\omega_{p2}$..
